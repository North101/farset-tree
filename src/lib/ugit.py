...
class Github:
  ...
  def pull_file(self, path: str, git_path: str | None = None):
    try:
      r = urequests.get(f'{self.raw_url}{git_path or path}', headers=self.headers)
      self.mkdir(path)
      with open(path, 'wb') as f:
        f.write(r.content)
    except:
      self.print(f'Failed to download: {path}')

  def pull(self, root: str | None = None, ignore: list[str] | None = None, ignore_dot_files=True):
    self.print(f'Pulling repo: {self.raw_url}')

    root = self.normalize_path(root if root else '', is_dir=True)
    ignore = [
    ...
    for git_path, git_hash in self.list_git_tree():
      if not git_path.startswith(root):
        continue

      path = git_path[len(root)-1:]
      if self.is_ignored(ignore, path, ignore_dot_files):
        self.print(f'{"Ignored":<9} {path}')
        continue

      file_hash = self.hash_file(path) if path in files else None
      ...
      # Don't delete files from github repo
      if path in files:
        files.remove(path)
  ...
  def list_files(self, path: str):
    for name, type, *_ in os.ilistdir(path):
      ...
      if type & 0x4000:
        yield from self.list_files(sub_path)
      else:
        yield sub_path
  ...
  def join_path(self, path: list[str], trailing=False):
    return f'/{"/".join(path)}{"/" if path and trailing else ""}'
...
def pull(user: str, repo: str, ref: str, token: str | None = None, root: str | None = None, ignore: list[str] | None = None, ignore_dot_files=True):
  return Github(
      user=user,
      repo=repo,
      ref=ref,
...
def pull_file(user: str, repo: str, ref: str, path: str, git_path: str | None = None, token: str | None = None):
  return Github(
      user=user,
      repo=repo,
      ref=ref,
